...
for Y in range(st_year, ed_year):
	...
	fvol = utils.fvolatility_prefer(data=fdata, year=str(Y), pnum=pnum)
	_trade_info['fcode'] = fvol.index
...
trade_info.to_pickle('data/vol_strategy_trade_info.pkl')
...
class VolatilityStrategy(bt.Strategy):
	...
	def __init__(self):
		...
		#                                    等权重
		# =====================================================================
		self.buy_fund = pd.read_pickle("data/vol_strategy_trade_info.pkl")
...
ret.to_pickle('data/volatility_strategy_returns.pkl')
...
qs.reports.html(returns=ret,
                benchmark=ben,
                title='Volatility Strategy Tearsheet',
                output='EquityFundAnalysis\reports',
                download_filename='VolatilityStrategyReport.html',
