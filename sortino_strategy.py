...
for Y in range(st_year, ed_year):
	...
	fsortino = utils.fsortino_prefer(data=fdata, year=str(Y), pnum=pnum)
	_trade_info['fcode'] = fsortino.index
...
trade_info.to_pickle('data/sortino_strategy_trade_info.pkl')
print("--------------------调仓列表已完成--------------------")
...
class SortinoStrategy(bt.Strategy):
	...
	def __init__(self):
		...
		#                                    等权重
		# =====================================================================
		self.buy_fund = pd.read_pickle("data/sortino_strategy_trade_info.pkl")
...
cerebro.addstrategy(SortinoStrategy)
...
qs.reports.html(returns=ret,
                benchmark=ben,
                title='Sortino Strategy Tearsheet',
                output='EquityFundAnalysis\reports',
                download_filename='SortinoStrategyReport.html',
